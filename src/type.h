...
private:
    ...
    // We don't store name here because for compound types it might be tricky to get them.
};
...
public:
    enum class IntTypeID {
        BOOL,
        // We don't have "char" as a type, because its signedness is implementation-defined
        SCHAR,
        UCHAR,
        SHORT,
        USHORT,
    ...
    // These utility functions take IntegerTypeID and return shared pointer to corresponding type
...
public:
    TypeBool(bool _is_static, CVQualifier _cv_qual) : IntegralTypeHelper(_is_static, _cv_qual) {}
...
public:
    TypeSChar(bool _is_static, CVQualifier _cv_qual) : IntegralTypeHelper(_is_static, _cv_qual) {}
